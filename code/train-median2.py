...
def train(dataset):
    ...
    lowerb = 20
    upperb = min(500, len(X_train.columns))

    if args.dataset == 'eco' or args.dataset == 'summary-eco':
        lowerb = 5
    ...
    if args.dataset == 'summary':
        ...
        param_grid = {
            'select__max_features': [int(x) for x in np.linspace(start = lowerb, stop = upperb, num = 25)],
            'rf__n_estimators': [int(x) for x in np.linspace(start = 100, stop = 1000, num = 10)],
            'rf__max_features': ['sqrt', 'log2'],
            'rf__max_depth': [None, 3]
        }
